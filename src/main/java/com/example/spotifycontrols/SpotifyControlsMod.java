...
public class SpotifyControlsMod implements ClientModInitializer {
    ...
    public void onInitializeClient() {
        ...
        // Try to auto-refresh expired tokens (must be called AFTER spotifyAuth exists)
        tokenStorage.tryAutoRefresh();
    ...
    private void updateXpBar(MinecraftClient client) {
        ...
        // save the real progress value once, before we start overwriting
        if (!xpSaved) {
            savedXpProgress = client.player.experienceProgress;   // float field
            xpSaved         = true;
        }

        // overwrite ONLY the progress bar (0.0 – 1.0) with song progress
        // leave experienceLevel alone so player's real level shows
        client.player.experienceProgress = currentProgress;
    }

    /** Restore real XP progress when music stops / pauses. */
