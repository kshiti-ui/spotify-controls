...
public class TokenStorage {
    ...
    /**
     * Attempts to auto-refresh the access token if it's expired but we
     * still have a valid refresh token.  This is called by the mod after
     * all components are initialized.
     * 
     * This prevents forcing the user to re-login every session when the
     * access token expires (after ~1 hour) but the refresh token is still
     * valid (typically valid for much longer).
     */
    public void tryAutoRefresh() {
        if (refreshToken == null) {
            return;  // no refresh token → user must login manually
        }
        
        if (!isTokenExpired()) {
            return;  // access token still valid, nothing to do
        }
        
        // Access token expired but we have a refresh token → attempt refresh
        SpotifyControlsMod.LOGGER.info("[SpotifyControls] Access token expired, attempting auto-refresh...");
        
        try {
            SpotifyControlsMod.getSpotifyAuth().refreshToken();
            SpotifyControlsMod.LOGGER.info("[SpotifyControls] Auto-refresh succeeded ✓");
        } catch (Exception e) {
            SpotifyControlsMod.LOGGER.warn("[SpotifyControls] Auto-refresh failed: " + e.getMessage());
            SpotifyControlsMod.LOGGER.warn("[SpotifyControls] Please run /spotify login again");
            // Clear expired tokens so user gets a clean "not logged in" error
            clearToken();
        }
    }
}
